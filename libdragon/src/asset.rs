...
pub fn load<T>(path: dfs::DfsPathBuf) -> Option<Box<[T]>> {
    ...
    unsafe {
        ...
        // Create a slice
        let slice: &mut [T] = core::slice::from_raw_parts_mut(ptr as *mut _, (sz as usize) / core::mem::size_of::<T>());
...
pub fn fopen(path: dfs::DfsPathBuf) -> Result<dfs::File> {
    let path_bytes: &[u8] = path.as_bytes();
    let cpath = CString::new(path_bytes).unwrap();
